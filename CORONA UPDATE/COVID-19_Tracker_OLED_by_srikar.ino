/*  Brought to you by hobbby kit  // youtube link == https://www.youtube.com/channel/UCnMtJhEpJLgQruS7kRElyeg  
 ...
 */
 ...
 #include "images.h"  // Include custom images
  ...
  const char* ssid = "hobby kit";              //WIFI SSID Name                             
  const char* password =  "1234567890";        //WIFI Password
  const char* host = "api.thingspeak.com";  //We read the data from this host                                   
  const int httpPortRead = 80;                                           
                                                  
  const char* url1 = " https://api.thingspeak.com/apps/thinghttp/send_request?api_key=RV2EAATOLUBLSQWK";     //Change this URL Cases
  const char* url2 = " https://api.thingspeak.com/apps/thinghttp/send_request?api_key=DDYQHWYI85HAME80";   //Deaths
  const char* url3 = " https://api.thingspeak.com/apps/thinghttp/send_request?api_key=XK8DR6C53IX29Y64";   //Recovered
  ...
  void drawFrame5(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
   ...
   display->setTextAlignment(TEXT_ALIGN_CENTER);
   display->setFont(ArialMT_Plain_16);
   display->drawString(64,10, "subscribe");
   display->drawString(64, 35, "hobby kit");
    }
     void drawFrame7(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
   display->clear();
   display->setTextAlignment(TEXT_ALIGN_CENTER);
   display->setFont(ArialMT_Plain_16);
   display->drawString(64,10, "stayhome");
   display->drawString(64, 35, "stay safe");
     }
